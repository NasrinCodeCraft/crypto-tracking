...
const Navbar = () => {
    ...
    const inputHandler = (event) => {
        ...
        if (value == "") {
            ...
            const suggestion = cryptoList.filter((coin) => coin.name.toLowerCase().includes(value.toLowerCase()))
            setFilteredCoins(suggestion.slice(0, 5))
        }
    }

    return (
        <nav className="flex flex-wrap md:flex-nowrap items-center justify-between gap-4 px-[5%] md:px-[8%] lg:px-[10%] py-5 bg-gray-900/80 backdrop-blure-md border-b border-gray-700/30 sticky top-0 z-50">
            ...
            <form onSubmit={searchHandler} className='order-3 w-full md:order-2 md:w-auto flex-1 max-w-2xl mx-0 md:mx-4 relative'>
                <div className='relative group'>
                    <div className='absolute -inset-0.5 bg-gradient-to-r from-emerald-600/40 to-cyen-500/40 rounded-full
                    ...
                    <div className='relative flex items-center'>
                        ...
                        <button type='submit' className='z-10 relative right-2 top-1/2 -translate-1/2 px-4
                ...
                {filteredCoins.length > 0 && (
                    <ul className='absolute w-full bg-gray-800/95 border border-gray-700 mt-2 rounded-lg shadow-xl
